...
export default function ProjectDetail() {
  ...
  return (
    ...
    >
      <Link
        to="/"
        className="inline-flex items-center text-newsroom-crimson hover:text-white transition-colors mb-10 uppercase tracking-widest font-bold text-sm"
      >
      ...
      <div className="mb-12">
        ...
        {/* Description */}
        <div className="prose prose-invert prose-lg max-w-none font-serif text-neutral-300 leading-relaxed mb-8">
          <p className="text-xl md:text-2xl leading-relaxed text-neutral-200 border-l-4 border-newsroom-crimson pl-6 py-2">
            {project.context}
          </p>
        </div>

        {/* ── Metadata Bar (ROLE / LOCATION / DATE / GEAR) ── */}
        <div className="grid grid-cols-2 md:grid-cols-4 border border-neutral-800 divide-x divide-y md:divide-y-0 divide-neutral-800 mb-8">
          {[
            { label: 'ROLE',     value: project.role     ?? '—' },
            { label: 'LOCATION', value: project.location          },
            { label: 'DATE',     value: project.date               },
            { label: 'GEAR',     value: project.gear     ?? '—' },
          ].map(({ label, value }) => (
            <div key={label} className="flex flex-col gap-1 px-5 py-4">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-newsroom-crimson">
                {label}
              </span>
              <span className="text-sm font-mono text-white leading-snug">
                {value}
              </span>
            </div>
          ))}
      ...
      {project.videoUrl && (
        <div className="mb-12 w-full aspect-video relative border border-neutral-800 overflow-hidden rounded-sm shadow-2xl shadow-black">
          <iframe
        ...
        {project.images.map((img, i) => (
          ...
          >
            <img
              ...
              loading="lazy"
              decoding="async"
              className="w-full h-auto object-cover transition-all duration-500 group-hover:scale-105 brightness-90 group-hover:brightness-110"
            />
          </div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {lightboxIndex !== null && (
          ...
          >
            ...
            {/* Prev */}
            <button
              className="absolute left-6 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-white z-[110] hidden md:block transition-colors"
            ...
            {/* Next */}
            <button
              className="absolute right-6 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-white z-[110] hidden md:block transition-colors"
              onClick={(e) => { e.stopPropagation(); paginate(1); }}
            >
